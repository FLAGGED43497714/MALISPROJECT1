...
path_import_sh = 'super_clean_shuffled.csv'


path_out_sh_pred = 'SGD/resultsSGD.csv'
path_out_sh_pred_prob = 'SGD/probasSGD.csv'

path_out_final = 'SGD/resultsANDprobs'
...
if imported :
    dfclean = pd.read_csv(path_import_sh, delimiter=',', dtype = float)
    ...
    df_total_pred = pd.DataFrame(y_total_pred_prob)
    df_total_pred.to_csv(path_out_sh_pred_prob,index=False,header=False)
